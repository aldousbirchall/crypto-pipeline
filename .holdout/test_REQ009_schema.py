...
def test_existing_tables_not_dropped(run_cli, tmp_path):
    ...
    # Create the database with a pre-existing candle row
    conn = sqlite3.connect(str(db_path))
    conn.execute("""
    ...
    sentinel = conn.execute(
        "SELECT COUNT(*) FROM candles WHERE asset = 'sentinel'"
    ).fetchone()[0]
    conn.close()
...
def test_candles_table_has_required_columns(run_cli, tmp_path):
    ...
    required = {"asset", "period", "open", "high", "low", "close", "volume"}
...
def test_metrics_table_has_required_columns(run_cli, tmp_path):
    ...
    col_names = {c[1] for c in cols_info}
    required = {"asset", "period", "sma_20", "sma_50", "volatility", "vwap"}
    missing = required - col_names
