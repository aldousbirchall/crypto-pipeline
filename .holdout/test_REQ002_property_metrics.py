...
def test_sma_of_constant_prices_equals_constant(run_cli, price, tmp_path):
    ...
    metrics = conn.execute(
        "SELECT * FROM metrics WHERE asset = 'bitcoin' AND sma_20 IS NOT NULL ORDER BY period"
    ).fetchall()
    conn.close()
...
def test_volatility_of_constant_prices_is_zero(run_cli, price, tmp_path):
    ...
    conn.row_factory = sqlite3.Row
    metrics = conn.execute(
        "SELECT * FROM metrics WHERE asset = 'bitcoin' AND volatility IS NOT NULL ORDER BY period"
    ).fetchall()
    conn.close()
...
def test_sma20_bounded_by_window_min_max(run_cli, prices, tmp_path):
    ...
    candle_rows = conn.execute(
        "SELECT close FROM candles WHERE asset = 'bitcoin' ORDER BY period"
    ).fetchall()
    conn.close()
...
def test_vwap_uniform_volume_equals_avg_typical_price(run_cli, prices, tmp_path):
    ...
    conn.row_factory = sqlite3.Row
    metrics = conn.execute(
        "SELECT * FROM metrics WHERE asset = 'bitcoin' ORDER BY period"
    ).fetchall()
    candle_rows = conn.execute(
        "SELECT * FROM candles WHERE asset = 'bitcoin' ORDER BY period"
    ).fetchall()
    conn.close()
