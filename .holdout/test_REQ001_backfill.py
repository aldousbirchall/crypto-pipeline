...
def test_backfill_single_asset_populates_candles(run_cli):
    ...
    rows = conn.execute(
        "SELECT COUNT(*) FROM candles WHERE asset_id = 'bitcoin'"
    ).fetchone()[0]
    conn.close()
...
def test_backfill_default_assets(run_cli):
    ...
    assets = [
        ...
        for r in conn.execute(
            "SELECT DISTINCT asset_id FROM candles ORDER BY asset_id"
        ).fetchall()
    ]
    conn.close()
...
def test_backfill_candle_fields(run_cli):
    ...
    row = conn.execute("SELECT * FROM candles WHERE asset_id = 'bitcoin' LIMIT 1").fetchone()
    conn.close()
...
def test_backfill_no_duplicate_candles(run_cli):
    ...
    conn = sqlite3.connect(str(db_path))
    dupes = conn.execute("""
        SELECT asset_id, period, COUNT(*) as cnt
        FROM candles
