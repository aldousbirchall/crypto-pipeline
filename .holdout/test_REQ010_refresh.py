...
def test_refresh_incremental_fetch(run_cli, tmp_path):
    ...
    # populate_candles already creates the full schema via _ensure_schema
    conn = sqlite3.connect(str(db_path))
    initial_count = conn.execute("SELECT COUNT(*) FROM candles").fetchone()[0]
    latest_period = conn.execute(
        "SELECT MAX(period) FROM candles WHERE asset_id = 'bitcoin'"
    ).fetchone()[0]
    conn.close()
    ...
    conn = sqlite3.connect(str(db_path))
    final_count = conn.execute("SELECT COUNT(*) FROM candles WHERE asset_id = 'bitcoin'").fetchone()[0]
    conn.close()
...
def test_refresh_recomputes_metrics(run_cli, tmp_path):
    ...
    metrics_before = conn.execute(
        "SELECT COUNT(*) FROM metrics WHERE asset_id = 'bitcoin'"
    ).fetchone()[0]
    latest_period = conn.execute(
        "SELECT MAX(period) FROM candles WHERE asset_id = 'bitcoin'"
    ).fetchone()[0]
