...
def test_stream_stores_tick_in_database(run_cli_subprocess, tmp_path):
...
def test_stream_ctrl_c_exits_cleanly(run_cli_subprocess, tmp_path):
...
def test_stream_multiple_assets_separate_records(run_cli_subprocess, tmp_path):
    ...
    if len(rows) > 0:
        assets_seen = set(r["asset_id"] for r in rows)
        # Each row should have its own asset field
        for r in rows:
            assert r["asset_id"] is not None, "Tick record missing asset name"
